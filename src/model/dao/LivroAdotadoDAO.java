...
public class LivroAdotadoDAO {
	...
	public void update(LivroAdotado la) {
		...
		try {
			stmt = con.prepareStatement(
					"UPDATE LivroAdotado SET numDiscipl = ?, Semestre = ?, ISBNLivro = ? WHERE numDiscipl = ?");
			stmt.setString(1, la.getNumDiscipl());
			stmt.setString(2, la.getSemestre());
			stmt.setString(3, la.getISBNLivro());
	...
	public void delete(LivroAdotado la) {

		Connection con = ConnectionFactory.getConnection();
		PreparedStatement stmt = null;

		try {
			stmt = con.prepareStatement("DELETE FROM LIVROADOTADO WHERE NumDiscipl = ? AND Semestre = ?"
					+ " AND ISBNLivro = ?");
			stmt.setString(1, la.getNumDiscipl());
			stmt.setString(2, la.getSemestre());
			stmt.setString(3, la.getISBNLivro());
